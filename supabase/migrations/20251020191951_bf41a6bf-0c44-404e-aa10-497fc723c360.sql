...
CREATE TABLE IF NOT EXISTS public.profesionales_clinicos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP TRIGGER IF EXISTS update_profesionales_clinicos_updated_at ON public.profesionales_clinicos;
...
DROP POLICY IF EXISTS "Admins de clínica pueden ver su clínica" ON public.clinicas;
...
DROP POLICY IF EXISTS "SuperAdmin puede gestionar clínicas" ON public.clinicas;
CREATE POLICY "SuperAdmin puede gestionar clínicas"
ON public.clinicas FOR ALL
USING (public.has_role(auth.uid(), 'superadmin'));

DROP POLICY IF EXISTS "Admin de clínica puede actualizar su clínica" ON public.clinicas;
...
DROP POLICY IF EXISTS "Profesionales pueden ver su propio registro" ON public.profesionales_clinicos;
...
DROP POLICY IF EXISTS "Usuarios pueden crear su registro de profesional" ON public.profesionales_clinicos;
CREATE POLICY "Usuarios pueden crear su registro de profesional"
ON public.profesionales_clinicos FOR INSERT
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Profesionales pueden actualizar su registro" ON public.profesionales_clinicos;
...
DROP POLICY IF EXISTS "Profesionales pueden ver sus clínicas" ON public.clinica_profesionales;
...
DROP POLICY IF EXISTS "Admin de clínica puede gestionar profesionales" ON public.clinica_profesionales;
...
DROP POLICY IF EXISTS "Pacientes pueden ver sus clínicas" ON public.clinica_pacientes;
...
DROP POLICY IF EXISTS "Admin de clínica puede gestionar pacientes" ON public.clinica_pacientes;
